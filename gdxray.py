...
class XrayDataset(utils.Dataset):
    ...
    def load_gdxray(self, dataset_dir, subset, series, auto_download=False):
        ...
        # Add classes
        self.add_class(source="gdxray", class_id=CASTING_DEFECT,
                       class_name="Casting Defect")
        self.add_class(source="gdxray", class_id=WELDING_DEFECT,
                       class_name="Welding Defect")
    ...
    def create_mask(self, dataset_dir, image_id):
        ...
        for i, box in enumerate(info["annotations"]):
            ...
            rr, cc = draw.ellipse(center_y, center_x, r_y,
                                  r_x, shape=mask.shape)
            mask[rr, cc] = 1
            # Debugging
    ...
    def load_mask(self, image_id):
        ...
        class_ids = np.array(
            [CLASS for _ in range(len(masks))], dtype=np.int32)
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        average_precision = evaluate_gdxray(
            model, dataset_val, limit=args.limit)
        print("Got AP={} using {} images.".format(
            average_precision, args.limit))
    else:
